...
D31_data <- inner_join(Phycosphere_asv_D31, Phycosphere_tax) |> #Groups by otu automatically
  mutate(time = "D31",
         genus=str_replace_all(string=genus,
               pattern="Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium", replacement="A-N-P-R")) 
...
agg_genus_data_D3_color <- D3_data |> 
  ...
  mutate(color = if_else(condition = is.na(color), true = "black", false = color),
         ...
         asv = str_replace(string = asv, pattern = "0+", replacement = ''),
         time = "D3",
...
D31_plots_combined<- agg_genus_data_D31_color |> 
  mutate(subset = ifelse(rel_abund > 0.01, "Abundant",
  ...
  geom_text(aes(label = significant), 
              position = position_stack(vjust = 0.5), vjust = .8, size = 20 / .pt) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    labs( y = "Cumulative Relative Abundance (ASV)", x = "Amplicon Sequence Variant", fill = '',
