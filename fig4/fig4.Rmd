...
tree <- read.tree("../mothur_outputs/GRBC_seqs/final.pick.asv.braycurtis.ASV.tre")

to_keep <- tax_effects$Group

pruned_tree <- keep.tip(tree, to_keep)

p <- ggtree(pruned_tree, layout = "circular") %<+% tax_effects +
  geom_fruit(aes(y = label, fill = host_species),
             geom = geom_tile,
             width=0.05,
             pwidth = 0.1,
             color="white",
